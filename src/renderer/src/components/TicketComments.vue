...
<template>
  ...
  >
    <div class="w-full max-w-2xl max-h-[90vh] animate-in zoom-in-95 duration-200 flex flex-col rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900">
      ...
      <div class="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-xl shrink-0 dark:border-slate-700 dark:bg-slate-800/70">
        <div class="flex items-center gap-3">
          <div class="bg-blue-100 p-2 rounded-lg text-blue-600 dark:bg-blue-500/15 dark:text-blue-300">
        ...
        <button @click.stop="emit('close')" class="text-slate-400 hover:text-slate-600 p-2 rounded-lg transition-colors dark:text-slate-500 dark:hover:text-slate-300 dark:hover:bg-slate-700/50">
          <X :size="20" />
        </button>
      </div>
      ...
      <div class="p-6 overflow-y-auto flex-1 bg-white rounded-b-xl dark:bg-slate-900">
        <div class="flex flex-col gap-8">
          ...
          <div class="bg-slate-50 border border-slate-200 rounded-xl p-5 shadow-sm shrink-0 dark:bg-slate-800/40 dark:border-slate-700">
            <div class="space-y-4">
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  <button 
                    ...
                    type="button"
                    class="text-[11px] px-3 py-1.5 bg-white border border-slate-200 rounded-full hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-all shadow-sm active:scale-95 dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-blue-500/15 dark:hover:text-blue-300"
                  >
                    {{ tpl.label }}
                  </button>
                </div>
              ...
              <div>
                ...
                <input v-model="author" type="text" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500" />
              </div>
              <div>
                ...
                <textarea v-model="text" rows="3" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 focus:ring-2 focus:ring-blue-500 outline-none resize-none dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500" placeholder="Напишіть щось або виберіть шаблон..."></textarea>
              </div>
            </div>
          ...
          <div class="space-y-4">
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 pb-2 dark:text-slate-500 dark:border-slate-800">
